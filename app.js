    ...
    function applyI18n() {
        ...
        // data-i18n-aria: بيترجم خاصية aria-label (لقارئات الشاشة) - كانت ناقصة
        // فكل أزرار الأيقونات (فتح/قفل القائمة، مسح البحث، إخفاء البانرات...) كانت
        // فاضلة عربي دايماً مهما غيّرت لغة الواجهة.
        ...
        // نفس الفكرة لتلميح زرار الوضع الفاتح/الداكن (مش data-i18n عادي لأن نصه
        // بيتغيّر كمان حسب الوضع الحالي مش بس اللغة) - نعيد حسابه من الكلاس الحالي.
    ...
    function applyTheme(theme) {
        ...
        // ملحوظة i18n: النص ده كان متثبت عربي جامد هنا - دلوقتي بيتحدد حسب لغة
        // الواجهة الحالية (currentUiLang) عن طريق uiStr()، وapplyI18n() بتنادي
        // applyTheme() تاني بعد كل تغيير لغة عشان التلميح يتحدث فورًا.
    ...
    // ============ "أونلاين دلوقتي" في لوحة الأدمن — لازم نبعت نبضة (ping) دورية
    // للسيرفر عشان يعرف إن المستخدم لسه فاتح الموقع فعلاً. من غيرها، عمود "أونلاين"
    // في لوحة الأدمن هيفضل فاضي دايمًا لأي حد مهما كان فاتح الموقع فعليًا. ============
    ...
    // نوقف النبضة لو المستخدم سيّب التبويب/قفل الشاشة (Tab غير ظاهر) ونرجّعها
    // فورًا لما يرجع - عشان "أونلاين" يكون معبّر عن الاستخدام الفعلي مش بس تبويب مفتوح في الخلفية.
    ...
    // ============ حساب موقوف: بيتفعّل لايف على users/{uid}/suspended - أول ما الأدمن يوقف
    // الحساب، اللي فاتح الموقع دلوقتي بيتحوّل فورًا لشاشة "الحساب موقوف" حتى لو مقفلش
    // ويفتحه تاني، ومن غيرها (لو الحساب شغال عادي) الشاشة دي متبانش خالص ============
    ...
    function getBestBrowserVoice(gender) {
        ...
        // ⚠️ مهم: لو مفيش صوت فعلي بنفس اللغة (مثلاً مفيش صوت عربي متثبت على الجهاز/المتصفح -
        // ده شائع جدًا في متصفحات موبايل كتير)، بنرجّع null صراحةً بدل ما نختار صوت بلغة
        // تانية (زي إنجليزي) ونخلي المتصفح "يتلعثم" في قراءة نص عربي بصوت إنجليزي غلط أو
        // ميتكلمش خالص. الاستدعاء في speakText() هيستخدم Edge TTS (صوت عربي حقيقي من
        // السيرفر) في الحالة دي بدل ما يفضل يحاول بصوت المتصفح الغلط.
    ...
    // ============ لغة مخرجات أدوات الذكاء الاصطناعي (كل الأدوات ما عدا شات
    // يسر Pro بوت اللي ليه دالته الخاصة getAssistantSystemPrompt) ============
    // بتتحط في آخر الـ system prompt بتاع كل أداة "مولّدة" (تقرير أداء، خطة
    // تطور، سيرة ذاتية، بورتفوليو، رسالة تقديم، إيميل تدريب، نصايح، الخ) عشان
    // ناتجها يتبع لغة الواجهة الحالية (currentUiLang) بدل ما يفضل عربي دايماً
    // مهما غيّر المستخدم اللغة. الأدوات اللي بتعالج نص/صوت المستخدم نفسه
    // (التدقيق الأكاديمي، التلخيص، تفريغ الصوت) مستثناة عمداً لأن ناتجها
    // المفروض يفضل بنفس لغة النص الأصلي المُدخل عشان يفضل مفيد (تصحيح مقال
    // إنجليزي بالعربي مثلاً مالوش لازمة)، وأداة المقابلة الصوتية نفسها
    // مستثناة برضه لأن لغتها بتتحدد يدوياً من "شخصية المحاور" مش من الواجهة.
