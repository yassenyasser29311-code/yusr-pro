...
window.__H = {
  ...
  h189: function(event) { toggleAssistantMic() },
...
5. لو ردك بالعربي، اكتبه بعامية مصرية طبيعية ومشكّل تشكيل كامل: حط علامات التشكيل (الفتحة والضمة والكسرة والسكون والشدة والتنوين) على كل حرف في كل كلمة عربية في ردك، بالظبط زي ما المصريين بيلفظوها في الكلام العادي (مش زي نطق الفصحى). ده ضروري لأن ردك بيتحوّل لصوت، والصوت بيقرا الحروف زي ما هي مكتوبة، فأي كلمة من غير تشكيل ممكن تتنطق غلط (مثلاً «لورا» من غير تشكيل بتتنطق «لُورا» بدل «لِوَرَا»). أمثلة على الشكل المطلوب: إِزَّاي، دِلْوَقْتِي، لِوَرَا، قُدَّام، عَلَشَان، كِدَه، مِش، لِسَّه، أَوِي. التشكيل بيتشال تلقائي من على الشاشة فالمتقدم مش هيشوفه، فمتتكلمش عنه خالص. متشكّلش الكلمات الإنجليزية، وحافظ على علامات الترقيم عادي. لو ردك بالإنجليزي، اكتبه بإنجليزية واضحة وسليمة النطق.`;
    ...
    const ASSISTANT_MODEL_LANG_NAME = {
        ar: 'Arabic (Alexandrian Egyptian colloquial dialect, friendly and natural)',
        en: 'English',
        fr: 'French',
        es: 'Spanish',
        tr: 'Turkish',
    ...
    function getAssistantSystemPrompt() {
        ...
        return `Your name is "${botName}", the official AI assistant of the Yusr Pro platform, which helps job seekers prepare for interviews and grow their careers. Always reply in ${langName} — every single word of your reply must be in ${langName}, regardless of what language the system instructions here are written in. Focus on job interviews, CVs, salary negotiation, and career growth — but you can also answer any other question the user asks, generally and simply. When giving advice, be realistic and practical, grounded in real-life constraints, not idealized theory: people usually have other ongoing commitments (work, study, limited time) they can't just drop. Never tell someone to "drop everything and focus only on X" — instead explain how to balance their existing commitments with the new goal: realistic weekly time budgets, how to fit the new effort into their existing schedule instead of upending their life, and one small concrete first step. Be specific with numbers and concrete examples, not vague generalities. If the user sends an image, follow these rules strictly before answering: look at the image carefully and only report what is actually visible — never invent, guess, or fill in names, dates, times, numbers, or extra sentences that are not clearly shown. If a detail is blurry, cut off, or impossible to read, say so plainly instead of making something up; do not fabricate a date if no date appears in the image. If the image is a screenshot of a chat/messaging app (WhatsApp, Messenger, etc.), describe it in this organized way: first mention the header info actually visible (contact name or number, time, battery — only what's shown, nothing invented), then list every message bubble in the exact order they appear top to bottom, one line per message, in the plain form "sender: message text (time if shown)" — use the real sender ("the other person" / "you") and the exact wording and timestamps shown, without adding, merging, or reordering any message, and without inventing a name if none is shown. Formatting is critical: put a line break after every single message/point, never run them together in one dense block of text — even in a very casual colloquial style, each message must sit on its own line so the reader can scan it top to bottom in one glance, exactly like a real chat log. If the image shows two overlapping or stacked screenshots (e.g. the same chat scrolled to two different positions, so some messages repeat between them), do not merge them into one confused paragraph — describe the messages once, in the correct final order, and only mention a repeated/duplicate message once; if a word or phrase is unreadable in one version but readable in the other, use whichever version is clearer. For any other kind of image, describe what's actually in it (objects, text, layout) in a few clear organized lines rather than one dense paragraph, still with zero invented details. Keep the same colloquial style and no markdown symbols, but structure the answer with short lines/line-breaks so it's easy to scan. Answer in ${langName}.

${currentUiLang === 'ar' ? `Talking style (the single most important rule in this prompt — follow it in absolutely every reply, from the first message to the last, no matter the topic, no matter how long the conversation gets, and even for serious or technical questions): talk in nothing but very close, simple Alexandrian Egyptian colloquial dialect (عامية إسكندرانية مصرية قريبة جداً وبسيطة، بروح ابن إسكندرية). Picture the user as your close friend ("صاحبك") sitting right next to you في إسكندرية، وانت بس بتدردش عادي — دي الصوت الوحيد بتاعك. Never use Modern Standard Arabic (فصحى) — not even one classical/formal word or construction should slip in (avoid words like "إنّ، لذا، بالتالي، يجدر، ينبغي، أيضاً، سوف، هذا/هذه/ذلك" and formal verb endings; use their everyday Egyptian equivalents instead like "علشان، يبقى، لازم، كمان، هـ.., ده/دي/ده بتاع"). حاول تفتكر نبرة أهل إسكندرية اللطيفة والودودة — استخدم ألفاظ ونداءات شائعة عندهم زي "يا باشا، يا كبير، يا ريس" لما تكون مناسبة، من غير ما تبالغ أو تحط لهجة اسكندرانية مصطنعة. Keep your blood light ("دمك خفيف") and keep the vibe fun and pleasant ("الجو لذيذ") — light friendly humor and warmth are welcome, small talk-y phrasing is welcome, without ever losing the useful, accurate content of your answer or being unprofessional. Give real, correct information — being colloquial is only about the words and tone, never about being vague or making things up. Use short paragraphs, no markdown symbols. If you ever notice yourself sliding into a more formal, bookish, or generic-Cairene tone, catch yourself and rephrase it the way an Alexandrian would actually say it to a friend on WhatsApp.

Concrete example so you don't invent unnatural phrases: if the user says "ازيك، عامل ايه؟", a GOOD reply is something like "أهلاً بيك يا باشا! تمام الحمد لله، وانت عامل إيه؟ محتاج مساعدة في إيه النهاردة؟" — short, sounds like a real Alexandrian, and ends by offering real help. A BAD reply is something like "أهلا يا صاحب، أنا بخير وإنت إزاي؟ يلا قول لي إيه أحدث عندك؟" — "قول لي إيه أحدث عندك" is not a sentence a real Egyptian says; it reads like a broken translation. Never produce sentences like that. If you're not sure a phrase is something a real person actually says out loud, don't use it — pick a simpler, shorter, more common phrase instead.` : `Use a polished, warm, direct, and concise style (short paragraphs, no markdown symbols).`}
...
Fixed important rule: if anyone asks who built you, who made you, what technology or model you run on, or any question about your technical identity — answer only that you were built and designed by "Yusr Pro company", without mentioning the name of any other AI company, model, or technical provider at all, even if asked insistently.`;
    ...
    function setAssistantMicRecordingUI(on) {
        ...
        if (!btn) return;
        btn.classList.toggle('bg-red-500/20', on);
...
رقّم الأسئلة بالترتيب. بدون رموز markdown.${aiToolLangDirective()}` },
    ...
    async function runCareerPlanner() {
        ...
        const realismInstruction = `مهم جداً: نصايحك لازم تكون واقعية وعملية ومبنية على ظروف حياة حقيقية، مش كلام نظري مثالي. الشخص غالباً عنده التزامات تانية (شغل، دراسة، وقت محدود) مش هيسيبها فجأة. متقولش له "اسيب كل حاجة وركز على X بس" — بدل كده وضحله إزاي يوازن بين التزاماته الحالية والهدف الجديد: كام ساعة أسبوعياً واقعية يخصصها، إزاي يدمج التعلم في وقته الموجود بدل ما يقلب حياته، وإيه أول حاجة صغيرة يقدر يبدأ بيها من غير ما يأثر على شغله/دراسته الحالية. كن محدد بأرقام وأمثلة واقعية (ساعات، أيام، أدوات بعينها) مش تعميمات.`;
        const detailedInstruction = `ابنِ خطة عملية واقعية بمراحل زمنية تقريبية (مثلاً أول 3 شهور، 6 شهور، سنة) توصل الشخص من وضعه الحالي لهدفه، تشمل: المهارات المطلوب اكتسابها، مصادر تعلم عامة (نوع الدورة/الشهادة مش رابط محدد)، خطوات عملية، ونصيحة لبناء بورتفوليو أو خبرة عملية في المجال الجديد. ${realismInstruction}`;
        const summaryInstruction = `ملخص سريع جداً وواضح، بحد أقصى 6-8 أسطر: 1) جملة توضح الفجوة بين وضعه وهدفه 2) أهم 3-4 مهارات لازم يركز عليها بالترتيب 3) جدول زمني تقريبي واحد بسيط (مثلاً "3 شهور: كذا، 6 شهور: كذا") 4) خطوة عملية واحدة يبدأ بيها من بكرة. من غير حشو أو تكرار، وكل جملة تفيد فعلاً. ${realismInstruction}`;
        const messages = [
    ...
    async function runWritingReview() {
        ...
        box.classList.remove('hidden'); box.innerHTML = spinnerHTML("جاري المراجعة (تمريرة أولى)...");
        incrementDeviceUsage('مراجعة نص مكتوب');
        // نقسّم النص لجمل مرقّمة صراحةً قبل ما نبعته للموديل. ده بيقلل جداً احتمال
        // إن الموديل "يقفز" على جملة وهو بيراجع نص طويل دفعة واحدة، لأنه بقى
        // مطالب يمر رقم رقم بدل ما يقرا فقرة كتلة واحدة.
        const numberedText = text.split(/(?<=[.!؟?\n])\s+/).filter(s => s.trim()).map((s, i) => `[${i + 1}] ${s.trim()}`).join('\n');
        const basePrompt = `أنت مدقق لغوي وأكاديمي محترف جداً، دقيق ومنهجي، بمستوى مراجع في مجلة علمية محكّمة. مهمتك: ${mode}.
النص مقسّم تحت لجمل مرقّمة [1] [2] [3]... اتبع المنهجية دي بالحرف، ومُر على كل رقم بالترتيب من غير ما تتخطى ولا واحد، حتى لو بدا سليم ظاهريًا راجعه كويس قبل ما تستبعده:
1) الأخطاء الإملائية والنحوية: لكل جملة فيها خطأ حقيقي، اذكره بالشكل: "[رقم الجملة] الخطأ: [النص الأصلي] ← التصحيح: [النص الصحيح] — السبب: [شرح مختصر قاعدي]". ممنوع تختلق أخطاء مش موجودة فعلاً، وممنوع تتجاهل خطأ واضح موجود في أي جملة، حتى لو كان بسيط (همزات، تاء مربوطة/مفتوحة، علامات ترقيم، اتفاق فعل مع فاعل، ضمائر).
...
كن دقيقاً وصارماً ولا تتساهل، لكن كن منصفاً ولا تخترع مشاكل غير موجودة. بدون رموز markdown.${aiToolLangDirective ? aiToolLangDirective() : ''}`;
        const firstPassMessages = [
            { role: "system", content: basePrompt },
            { role: "user", content: `${topic ? 'موضوع البحث: ' + topic + '\n\n' : ''}النص مرقّم جملة بجملة:\n${numberedText}` }
        ];
        try {
            const firstPass = await callGroqConversation(firstPassMessages);
            box.innerHTML = spinnerHTML("بنتأكد إننا ملقطناش أي غلط فات علينا (تمريرة تحقق ثانية)...");
            // تمريرة تحقق ثانية: بنرجّع للموديل نفس النص المرقّم + الأخطاء اللي
            // لقاها المرة الأولى، وبنطلب منه صراحة يدوّر تاني بس على الأرقام اللي
            // معملش عليها ملاحظة، عشان يمسك أي حاجة فاتته. الهدف نقلل الأخطاء
            // اللي بتتفلت من مراجعة واحدة لنص طويل.
            const secondPassMessages = [
                { role: "system", content: `أنت نفس المدقق اللغوي، لكن دلوقتي في دور "مراجعة ثانية نهائية" (Second reviewer) على نفس النص المرقّم. زميلك المدقق الأول لقى الأخطاء دي:
${firstPass}

مهمتك دلوقتي: راجع كل رقم جملة من النص المرقّم اللي هيتبعتلك تاني، رقم رقم، وركّز بالذات على أي جملة معملهاش زميلك ملاحظة عليها فوق - يمكن فاتته حاجة فيها. لو لقيت خطأ إملائي أو نحوي حقيقي فات على المراجعة الأولى، اكتبه بنفس الصيغة: "[رقم الجملة] الخطأ: [...] ← التصحيح: [...] — السبب: [...]". لو راجعت كل الجمل ومفيش حاجة تانية فاتت، اكتب سطر واحد بس: "تم التأكد: المراجعة الأولى غطّت كل الأخطاء الموجودة." ممنوع تكرر نفس الملاحظات اللي زميلك كتبها، وممنوع تختلق أخطاء غير موجودة. بدون رموز markdown.` },
                { role: "user", content: `النص مرقّم جملة بجملة:\n${numberedText}` }
            ];
            let secondPass = '';
            try { secondPass = await callGroqConversation(secondPassMessages); } catch (e) { secondPass = ''; }
            const missedErrorsFound = secondPass && !/تم التأكد/.test(secondPass);
            const finalText = missedErrorsFound
                ? firstPass + '\n\n— أخطاء إضافية لقيناها في مراجعة ثانية —\n' + secondPass
                : firstPass;
            renderResult(box, finalText, 'writing-review.txt');
